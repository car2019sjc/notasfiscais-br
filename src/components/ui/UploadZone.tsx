    ...
    <div 
      className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-all duration-300 transform hover:scale-105 ${
        isDragging 
          ...
          : 'border-gray-300 hover:border-orange-400 hover:bg-[#fba4120d]'
      }`}
      onDragEnter={(e) => { 
        e.preventDefault(); 
        e.stopPropagation(); 
      ...
      <label htmlFor={id} className="cursor-pointer flex flex-col items-center group">
        <FileSpreadsheet 
          className={`w-16 h-16 mb-4 transition-all duration-300 group-hover:scale-110 ${
            isDragging ? 'text-blue-800' : 'text-blue-800 group-hover:text-blue-900'
          }`} 
        />
        <span className="font-bold text-lg text-blue-800 mb-2 group-hover:text-blue-900 transition-colors duration-300">
          {title}
        </span>
